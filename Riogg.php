 ...
 /**
      ...
      * Date Updated V.8 : 07-November-2019
 ...
 **/
 ...
 class Marlboro extends modules
 {
     ...
     public function login($email, $password)
     {
         ...
         $post = 'email='.trim(urlencode($email)).'&password='.trim(urlencode($password)).'&ref_uri=/&decide_csrf='.$generateData[1].'&param=&exception_redirect=false';
     ...
     public function getPoint()
     {
         ...
         $points = $this->getStr($get[0], '/><div class="point">', '</div>', 1, 0);
         return trim($points);
     }
     ...
     public function execute_login($email, $password)
     {
         for($o=1;$o<=10;$o++)
         {
             ...
             if(strpos($login[0], '"code":200,"message":"success"'))
             {
                 ...
                 return false;
             }elseif(strpos($login[0], 'Akun Anda kami blokir')){
                 if(@file_exists($this->fileCookies) == true)
                 {
                     @unlink($this->fileCookies);
                 }
     ...
     public function execute_nonton($email)
     {
         ...
         {    
             sleep(1);
             @$pointAwal = $this->getPoint();
             $idVidio = $this->getStr($this->idVidio(), 'data-ref="https://www.marlboro.id/discovered/article/', '">', $b, 0);
             if(!empty($idVidio))
             {
     ...
     public function Passion()
     {
         ...
         $headers = explode("\n","Host: www.marlboro.id\nX-Requested-With: XMLHttpRequest\nContent-Type: application/x-www-form-urlencoded; charset=UTF-8\nCookie: deviceId=".$generateData[2]."; decide_session=".$generateData[0]);
         $post = 'answer=3&decide_csrf='.$generateData[1];
         $this->request($url, $post, $headers); $this->request($url, $post, $headers);
         if($pointAwal == $this->getPoint())
         {
             print PHP_EOL."Limit Get Point Passion.. ".$this->getPoint()." Pts";
         }else{
             print PHP_EOL."Sukses Get Point Passion.. ".$this->getPoint()." Pts";
         }
     }
 }
 ...
 if($menuk == "1")
 {
     ...
     while(true)
     {
         ...
         {    
             ...
             echo PHP_EOL.PHP_EOL.PHP_EOL."[".$c."] Ekse Akun : ".$email.PHP_EOL;
     ...
     {    
         ...
         $login = $marlboro->login($email, $password);
         if(strpos($login[0], '"code":200,"message":"success"'))
         {
             $point = $marlboro->getPoint();
             print "\n[".$c."] ".$email."|".$password." - ".$point." Pts";
             fwrite(fopen("marlboroCheckPoint.txt", "a"), $email."|".$password." - ".$point." Pts\n");
         }elseif(strpos($login[0], 'Akun Anda kami blokir')){
